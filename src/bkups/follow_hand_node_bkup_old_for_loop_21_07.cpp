...
//        for (int i = 0; i < vector->size(); i++){
//            printf(" %1.6f", vector->at(i));
//        }
    }
}

void calculateTrajectory(ros::NodeHandle * node, std::vector<double> * goalPosition, std::vector<double> * lastPosition, sensor_msgs::JointState * jointMsg, ros::Publisher * handJointPublisher, double * totalTime, double * publishRate, bool checkElbowRest){
    // time measurement
...
//    printVector(&deltaPosition, 0);
        ...
        //do{
            if(deltaPosition.at(i) < -M_PI){
                ...
                if(checkVel / *totalTime < maxVel) // maxVel not desired
                    ...
                    //return;
                    goalViolation = true;
                    printf("\n1:gehalten an Gelenk i= %d maxVel = %f, desiredVel = %f", i, maxVel, deltaPosition.at(i) / *totalTime);
                ...
                if(-maxVel < checkVel/ *totalTime) // maxVel not desired
                    ...
                    //return;
                    goalViolation = true;
                    printf("\n2:gehalten an Gelenk i= %d maxVel = %f, desiredVel = %f", i, maxVel, deltaPosition.at(i)/ *totalTime);
                    //printf("\n desired vel bigger velMax, wait for User to come back to robot.");
                    return;
                    // sleep(1);
                    // TODO: TEST THIS ! (should work but 17° /sec is super slow
    ...
    elapsedTime = ros::Time::now() - begin;
    printf("\nelapsed time for velocity restriction control: %f", elapsedTime.toSec());
    begin = ros::Time::now();
    ...
    */
    ...
    for (int i = 0; i <= numberPublishes; ++i) { // publish one more time to actually publish goalPosition
        ...
        if(node.ok()){
            ...
            for (int i = 0; i < interpolationvvectordofd[0].size(); ++i) {
                currentGoal.at(i) = interpolationvvectordofd[0](i);
            }
            ...
            calculateTrajectory(&node, &currentGoal, &lastPosition, &jointMsg, &handJointPublisher, &minPublishRate, &minPublishRate, checkElbowRest);
