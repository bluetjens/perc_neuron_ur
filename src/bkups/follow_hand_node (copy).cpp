...
// TODO: IMPLEMENT REINITIALIZATION PROCESS1!!!!!!!!
// TODO: IMPLEMENT QUESTION TO START INITIALIZATION PROCESS. _ YES OR NO
...
int calculateTrajectory(ros::NodeHandle * node, std::vector<double> * goalPosition, std::vector<double> * lastPosition, sensor_msgs::JointState * jointMsg, ros::Publisher * handJointPublisher, double * totalTime, double * publishRate, bool checkElbowRest){
    // time measurement
    ...
    if(checkElbowRest){
        double qInnerElbowMin = M_PI/9;
        double qInnerElbowMax = M_PI*8/9;
        ...
        if(qInnerElbowMin <= innerAngleElbow && innerAngleElbow <= qInnerElbowMax)
            goalPossible = true;
        else{
            goalPossible = false;
            goalViolation = true;
            printf("\n Desired goal is outside of elbow joint angle boundaries, wait for User to come back to robot.");
            return -1;
        }
...
//    printVector(&deltaPosition, 0);
    ...
    // ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    //TODO: ONLY FOR TESTING!!!,for real, eliminate this and take default value or pass parameter to calculateTrajectory()
        ...
        //do{
            if(deltaPosition.at(i) < -M_PI){
                ...
                if(-maxVel < checkVel/ *totalTime) // maxVel not desired
                    ...
                    goalPossible = false;
                    goalViolation = true;
                    printf("\n2:gehalten an Gelenk i= %d maxVel = %f, desiredVel = %f\nWait for user to come back to robot.", i, maxVel, deltaPosition.at(i)/ *totalTime);
                    return -1;
                    // TODO: TEST THIS ! (should work but 17° /sec is super slow
                }
            }
            // TODO CHECK NEGATIVE VALUES
                ...
                goalPossible = false;
                goalViolation = true;
                printf("\n3:gehalten an Gelenk i= %d maxVel = %f, desiredVel = %f\nWait for user to come back to robot.", i, maxVel, deltaPosition.at(i)/ *totalTime);
                // ROS_DEBUG_STREAM("\n3:gehalten an Gelenk i= %d maxVel = %f, desiredVel = %f" << i << maxVel << deltaPosition.at(i)/ *totalTime);
                return -1;
                // TODO: TEST THIS ! (should work but 17° /sec is super slow
            }
        //}
...
int initializeRobotToHand(std::vector<std::string>& bodyJointsRef, tf::TransformListener& listenerRef, tf::TransformBroadcaster& broadcasterRef, tf::Transform& transBaseLinkRightHandRef, tf::Transform& rotateRef)
{
    ...
    lastPosition = defaultRobotPosition;
    ...
    defaultPosMat << defaultRobotPosition.at(0), defaultRobotPosition.at(1), defaultRobotPosition.at(2), defaultRobotPosition.at(3), defaultRobotPosition.at(4), defaultRobotPosition.at(5);

    Eigen::Matrix<double, 6, 1>  goalvectordofd;
    goalvectordofd << qSolsArray[ikSolNr*6+0], qSolsArray[ikSolNr*6+1], qSolsArray[ikSolNr*6+2], qSolsArray[ikSolNr*6+3], qSolsArray[ikSolNr*6+4], qSolsArray[ikSolNr*6+5];
    ...
    ros::Rate initializationRate(1/minPublishRate);
    ros::Time initialTime;
    ros::Duration passedTime;
    ros::Time endTime;
    // ros::Duration minInitDuration = ros::Duration(5, 0.00); // Duration of initialization process
    double minInitDuration = 120; // initialization process needs 120 secs, default
    printf("\nminInitDuration in secs : %f", minInitDuration);
    node.getParam("/follow_hand/minInitializationDuration", minInitDuration);
    printf("\nminInitDuration in secs : %f", minInitDuration);

    initialTime = ros::Time::now();
    int numberPublishes = (int)floor(minInitDuration/minPublishRate);
    ...
    checkElbowRest = 0;
    publishedGoal = 1;
    // Calculate smooth trajectory to defaultRobotPosition and publish it to robot
    for (int i = 0; i <= numberPublishes; ++i) { // publish one more time to actually publish goalPosition
        ...
        if(node.ok()){
            ...
            // lastPosition gets updated by calculateTrajectory()
            ros::Time begin = ros::Time::now();
            ros::Duration elapsedTime = begin - begin;
            // TODO: ONLY FOR TESTING, ELIMINATE THIS:
            // minPublishRate = minPublishRate - 0.002; // time for calcTraj.
            publishedGoal = calculateTrajectory(&node, &currentGoal, &lastPosition, &jointMsg, &handJointPublisher, &minPublishRate, &minPublishRate, checkElbowRest);
            // check if goal already published reached. necessary because, calculateTrajectory needs more time than minPublishRate
            // TODO: eliminate this control
            int equalJoints = 0;
            for (int i = 0; i < lastPosition.size(); ++i) {
                if(lastPosition.at(i) == goalvectordofd[i]){
                    equalJoints++;
                }
            }
            if(equalJoints == 6){
...
int main(int argc, char** argv){
    ...
    double timeForMainLoop = 0.0006; // Time for MainLoop
    ...
    bool checkElbowRest = 1; // check boundaries of elbow joint in calculateTrajectory()
    // Interface to UR#
    // ^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    std::vector<double> defaultRobotPosition{0,-M_PI/9,M_PI/9,0,0,0};
    // std::vector<double> defaultRobotPosition{2.809080, -2.326490, -1.572308, 0.347548, 1.137317, -1.876026};
    std::vector<double> lastPosition = defaultRobotPosition;
    ...
    std::vector<double> ikSolution = defaultRobotPosition;
    /**     table for solutions:
...
//        }
        ...
        elapsedTime = ros::Time::now() - begin;
        //ROS_INFO_STREAM("elapsed time: " << elapsedTime);
        calculateTrajectory(&node, &ikSolution, &lastPosition, &jointMsg, &handJointPublisher, &totalLoopTime, &minPublishRate, checkElbowRest);
