...
struct ik{
    ...
    int calcqIKSolVec(const tfPercNeuronInterface& tfPNIntf){
        ...
        for(int i = 0;i<=2;i++){
            // rotational part of transf. matrix
            transForIK[i*4+0] = tfPNIntf.transBaseLinkRightHand.getBasis().getRow(i).getX();
            transForIK[i*4+1] = tfPNIntf.transBaseLinkRightHand.getBasis().getRow(i).getY();
            transForIK[i*4+2] = tfPNIntf.transBaseLinkRightHand.getBasis().getRow(i).getZ();
        }
        // translational part
        ...
        // Turn values over M_PI into negative values (by substraction of 2*M_PI) to use joint limited robot
...
/**     // Print all 8 solutions to inverse kinematics / joint positions
        ...
        for(int i = 0;i<numSols;i++){
            printf("%1.6f %1.6f %1.6f %1.6f %1.6f %1.6f\n",
                qIKSolArr[i*6+0], qIKSolArr[i*6+1], qIKSolArr[i*6+2], qIKSolArr[i*6+3], qIKSolArr[i*6+4], qIKSolArr[i*6+5]);

        }
        printf("\n");
//*/

        qIKSolVec = {qIKSolArr[ikSolNr*6+0], qIKSolArr[ikSolNr*6+1], qIKSolArr[ikSolNr*6+2], qIKSolArr[ikSolNr*6+3], qIKSolArr[ikSolNr*6+4], qIKSolArr[ikSolNr*6+5]};
        return 0;
    }
};
...
// Interpolates last published position with desired goal, checks physical constraints on robot and publishes new desired joint values to controller / robot
int checkAndPublishDesJointValues(ros::NodeHandle& node, std::vector<double>& goalPosition, std::vector<double>& lastPosition, pubIntf& pubIntf,
...
//    begin = ros::Time::now();
    ...
    if(interpolate){
        ...
        // printf("expected Cycle Time of checkAndPublishDesJointValues: %f", rate.expectedCycleTime().toSec());
...
int initializeRobotToHand(ros::NodeHandle& nodeRef, pubIntf& pubIntf, tfPercNeuronInterface& tfPNIntf,
                          ik& ik, std::vector<double>& lastPositionRef, std::vector<double>& defaultRobotPositionRef, const robotRestrictions& robRestrRef){
...
//            }
            ...
            ros::Time begin = ros::Time::now();
            ros::Duration elapsedTime = begin - begin;
            checkAndPublishDesJointValues(nodeRef, currentGoalRef, lastPositionRef, pubIntf, pubIntf.minPubRate, robRestrRef);
            // check if goal already published reached. necessary because, checkAndPublishDesJointValues needs more time than pubIntf.minPubRate
...
int main(int argc, char** argv){
    ...
    double timeForMainLoop = 0.0006; // Time for calculations in main loop without checkAndPublishDesJointValues()
...
//                 i = 7 : elbow up;    shoulder right;     wrist up / away from body
    */
    ...
    // Start the loop to get position from Perception Neuron and publish it to the robot / controller
    // ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
//*/
        ...
        int success = checkAndPublishDesJointValues(nodeRef, ik.qIKSolVec, lastPositionRef, pubIntfRef, totalTrajPubTime, robRestrRef);
