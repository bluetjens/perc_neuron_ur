...
//*/
        getTransBaseLinkRightHand(bodyJointsRef, listenerRef, broadcasterRef, transBaseLinkRightHandRef, &rotate);
        printf("\nData of transBaseLinkRighHandRef: %f %f %f", transBaseLinkRightHandRef.getBasis().getRow(i).getX(),
               transBaseLinkRightHandRef.getBasis().getRow(i).getY(), transBaseLinkRightHandRef.getBasis().getRow(i).getZ());
        while(node.ok()){
            rate.sleep();
        }
        //        // Get Transform / Vector from shoulder_link of ur5 to RightHand of PercNeuron
