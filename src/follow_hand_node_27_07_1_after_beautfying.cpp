...
// Looks up Transfrom from transOrigin to transDestination in tfListener and
...
int pushIntoTransformArray(std::string transOrigin,
  ...
  try {
    // TODO: look up: [ERROR] [1465987288.873699876]: "RightHand" passed to
...
struct tfPercNeuronInterface {
  std::vector<std::string> bodyJoints;  // Array with tf names of Perception
  ...
  int getTransBaseLinkRightHand() {
    ...
    for (int i = 0; i < bodyJoints.size() - 1; i++) {
      success += pushIntoTransformArray(
          bodyJoints.at(i), bodyJoints.at(i + 1), &transformArray,
    ...
    */
    ...
    // direction of the real hand/palm
    transBaseLinkRightHand =
...
struct ik {
  ...
  int calcqIKSolVec(const tfPercNeuronInterface& tfPNIntf) {
    ...
    // TODO: HANDLE CASE WHERE NO SOLUTION IS FOUND: robot arm should wait for
    // user to come back
    // waitForUser();
    ...
    /**     // Print all 8 solutions to inverse kinematics / joint positions
            ...
            for(int i = 0;i<numSols;i++){
                printf("%1.6f %1.6f %1.6f %1.6f %1.6f %1.6f\n",
                    qIKSolArr[i*6+0], qIKSolArr[i*6+1], qIKSolArr[i*6+2],
    qIKSolArr[i*6+3], qIKSolArr[i*6+4], qIKSolArr[i*6+5]);
...
int checkAndPublishDesJointValues(ros::NodeHandle& node,
  ...
  if (robRestrRef.checkElbowRestriction) {
    ...
    // ROS_INFO_STREAM("robRestrRef.innerElbowMin: "<<
    // robRestrRef.innerElbowMin);
    ...
    if (1 == violatesJointRestriction(robRestrRef, innerAngleElbow)) {
      goalPossible = false;
      printf(
          "\n Desired goal is outside of elbow joint inner angle boundaries: "
          "%f° - %f°, wait for User to come back to robot.",
    ...
    //        printf("\nelapsed time for joint restriction control: %f",
    //        elapsedTime.toSec());
    //        begin = ros::Time::now();
  }
  ...
  //    elapsedTime = ros::Time::now() - begin;
  //    printf("\nelapsed time for calculation of deltaPosition: %f",
  //    elapsedTime.toSec());
  //    begin = ros::Time::now();
  //    printf("\nlastPosition Vector: ");
  ...
  for (int i = 0; i < deltaPosition.size(); i++) {
    ...
    if (deltaPosition.at(i) < -M_PI) {
      ...
      if (checkVel / totalTrajTime < robRestrRef.maxVel)  // maxVel not desired
        ...
        goalPossible = false;
        // printf("\n1:Velocity boundary violation at joint: i= %d maxVel = %f,
        // desiredVel = %f\nWait for user to come back to robot.", i,
        // robRestrRef.maxVel, deltaPosition.at(i) / totalTrajTime);
        return -1;
      ...
      if (-robRestrRef.maxVel < checkVel / totalTrajTime)  // maxVel not desired
        ...
        goalPossible = false;
        // printf("\n2:Velocity boundary violation at joint: i= %d maxVel = %f,
        // desiredVel = %f\nWait for user to come back to robot.", i,
        // robRestrRef.maxVel, deltaPosition.at(i)/ totalTrajTime);
        return -1;
        // TODO: TEST THIS ! (should work but 17° /sec is super slow
    ...
    // TODO CHECK NEGATIVE VALUES
      ...
      goalPossible = false;
      // printf("\n3:Velocity boundary violation at joint: i= %d maxVel = %f,
      // desiredVel = %f\nWait for user to come back to robot.", i,
      // robRestrRef.maxVel, deltaPosition.at(i)/ totalTrajTime);
      return -1;
    }
  ...
  //    elapsedTime = ros::Time::now() - begin;
  //    printf("\n2:elapsed time for velocity restriction control: %f",
  //    elapsedTime.toSec());
  //    begin = ros::Time::now();
  ...
  if (interpolate) {
    ...
    // ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    // all solutions from here on should we realisable and not damaging the
    // robot
    // interpolate in  totalTrajTime ms to send in a rate of minPubRate (default
    // 8ms)
    // TODO: evaluate cycle Time from last published position to first one.
    // (should be 50ms - 40ms - ~3ms)
    ros::Rate rate(1 / pubIntf.minPubRate);  // should be 125 Hz = 1/8ms
    // printf("expected Cycle Time of checkAndPublishDesJointValues: %f",
    // rate.expectedCycleTime().toSec());

    // TODO: Shorten time to check boundaries, currently this needs
    // ~0.003+-0.001 secs which is pretty long
    ...
    for (int step = 1; step <= numberPublishes;
      ...
      //            elapsedTime = ros::Time::now() - begin;
      //            printf("\nelapsed time for publishing data in step %d: %f",
      //            step, elapsedTime.toSec());
      //            begin = ros::Time::now();
      //*/
    }
...
int initializeRobotToHand(ros::NodeHandle& nodeRef, pubIntf& pubIntf,
  ...
  getchar();
  // ros::Duration(2).sleep(); // wait until launch file started
  ...
  if (1 == violatesJointRestriction(robRestrRef, innerAngleElbow)) {
    printf(
        ...
        "desiredVal: %f",
        robRestrRef.innerElbowMin, robRestrRef.innerElbowMax, innerAngleElbow);
    return -1;
  }

  // static VVectorDOFd getJointPVT5(const VectorDOFd& start, const VectorDOFd&
  // goal, double t_current, double t_total);
  ...
  for (int i = 0; i <= numberPublishes;
       ++i) {  // publish one more time to actually publish goalPosition
               // TODO: eliminate if
...
int main(int argc, char** argv) {
  ...
  double framesPerSecPercNeuron = 120;
  // TODO: share Parameter with rosserial_server so that windows machine can
  // read the value!
  // node.setParam("/follow_hand/framesPerSecPercNeuron",
  // framesPerSecPercNeuron);
  // TODO: ELIMINATE THIS MAGIC VARIABLE
  double timeForMainLoop = 0.0006;  // Time for calculations in main loop
  ...
  // ^^^^^^^^^^^^^^^^^^^^^^^^^^^^
  // all ur5 values, including DH (Denavit-Hartenberg) values are stored on the
  // parameter server
  ...
  if (pubIntf.publishToController)
    ...
    // 1000); // publishes to "$roslaunch bringRobot demo.launch" starting a
    // simulation of ur5
    pubIntf.jointPosPublisher = node.advertise<sensor_msgs::JointState>(
  ...
  // ^^^^^^^^^^^^^^^^^^^^^^^^^^^^
  // Header header, string[] name, float64[] position, float64[] velocity,
  // float64[] effort, jointMsg-header... ros::Time(NOW)
  ...
  // const std::vector<std::string> = ur_kinematics::URKinematicsPlugin
  // kinplugin.getJointNames();
  ...
  // std::vector<tf::Transform> transformArray;
  // create string array with bodyJoints, TODO: assess all joints && prove if
  // order is correct
  tfPNIntf.bodyJoints = {
      "Hips", "Spine",         "Spine1",   "Spine2",       "Spine3",
      "Neck", "RightShoulder", "RightArm", "RightForeArm", "RightHand"};
  ...
  */
  ...
  robRestr.checkElbowRestriction =
      0;  // Elbow restrictions are only checked at the beginning of
  ...
  while (node.ok()) {
    ...
    //*/
    // Get Transform / Vector from shoulder_link of ur5 to RightHand of
    // PercNeuron
    // ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    success = -1;
    while (success == -1) {
    ...
    int success = checkAndPublishDesJointValues(nodeRef, ik.qIKSolVec,
    ...
    /** // Usage of ur_kinematics package:
    // ^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    double q[6] = {M_PI/12, M_PI/12, M_PI/12,M_PI/12 , M_PI/12, M_PI/12}; //
    joint space coordinates
    double* T = new double[16];
    ur_kinematics::forward(q, T); // calculates forward kinematics from joint
    position q (joint space coordinates) to goalTransform T (workspace
    coordinates)
    for(int i=0;i<4;i++) {
      for(int j=i*4;j<(i+1)*4;j++)
        printf("%1.3f ", T[j]);
    ...
    double q_sols[8*6]; // Matrix of all possible solution vectors in joint
    space, max number of solutions = 8; number of joints of ur# is 6
    int num_sols; // Number of possible analytic solutions of IK
    num_sols = ur_kinematics::inverse(T, q_sols); // calculates analytically all
    inverse kinematic solutions from goal transform matrix T to joint space
    q_sols; uses ur_kinematics package; to use ur3, ur5, ur10 edit ur_kin.cpp
    ...
    */
    ...
    /*      // display all possible joint positions
            for(int i = 0; i < ik.numSols; i++){
                pubIntf.jointMsg.header.stamp = ros::Time::now();
                pubIntf.jointMsg.position = {ik.qIKSolArr[i*6+0],
       ik.qIKSolArr[i*6+1], ik.qIKSolArr[i*6+2], ik.qIKSolArr[i*6+3],
       ik.qIKSolArr[i*6+4], ik.qIKSolArr[i*6+5]};
                pubIntf.jointPosPublisher.publish(pubIntf.jointMsg);
                rate.sleep();
            }
    */
